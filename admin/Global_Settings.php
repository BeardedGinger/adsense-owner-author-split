...
class Global_Settings extends \Genesis_Admin_Boxes {
	...
	function metaboxes() {
		...
		add_meta_box( 'aoas_default_settings', __( 'Defaults', 'owner_author_ad_split' ), array( $this, 'default_settings' ), $this->pagehook, 'main' );
		...
		<table class="form-table">
			<tbody>
				<tr valign="top">
					<th scope="row"></th>
					<td>
						<p class="description"><?php _e( 'By default ads show automatically on all posts. Alternatively, check this box and ads will be hidden by default and then can be enabled on a per-post basis.', 'owner_author_ad_split' ); ?></p>
					</td>
				</tr>
				...
				<tr valign="top">
					...
					<td>
						<p>
							<label for="<?php $this->field_id( 'hide_content_ads' ); ?>"><input type="checkbox" name="<?php $this->field_name( 'hide_content_ads' ); ?>" id="<?php $this->field_id( 'hide_content_ads' ); ?>" value="1" <?php checked( $this->get_field_value( 'hide_content_ads' ) ); ?> /><?php _e( 'Hide Content Ads by Default', 'owner_author_ad_split' ); ?></label>
						</p>
					</td>
				</tr>
